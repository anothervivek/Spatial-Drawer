    ...
    function onPinchStart(pinchPos){
        ...
        pinchStartDelay = new DoDelay(function(){
            ...
            if (script.worldCamera) {
                lookAtPos = script.worldCamera.getTransform().getWorldPosition();
            } else {
                try {
                    var WorldCam = require("SpectaclesInteractionKit.lspkg/Providers/CameraProvider/WorldCameraFinderProvider");
                    var cameraProvider = WorldCam.getInstance();
                    if (cameraProvider) {
                        lookAtPos = cameraProvider.getTransform().getWorldPosition();
                    }
                } catch (e) {}
            }
            if (!lookAtPos) {
